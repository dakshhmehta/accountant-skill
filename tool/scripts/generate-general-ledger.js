...
for (const [typeLabel, typeName] of Object.entries(typeLabels)) {
  ...
  const allTypeLedgers = ledgers.filter(l => l.type === typeLabel);
...
const tbData = db.prepare(`
  ...
  FROM ledgers l
  LEFT JOIN (
    SELECT vl.ledger_id, vl.debit, vl.credit
    FROM lines vl
    JOIN vouchers v ON v.id = vl.voucher_id
    WHERE v.status = 'POSTED' AND v.date <= '2026-05-31'
  ) vl ON vl.ledger_id = l.id
  GROUP BY l.id
